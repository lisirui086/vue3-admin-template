  ...
  <el-card style="margin: 10px 0">
    <el-form label-width="100">
      ...
      <el-form-item label="SKU描述">
        <el-input type="textarea" placeholder="请输入SKU描述" v-model="spuParams.description"></el-input>
      </el-form-item>
      <el-form-item label="SKU图片">
        <el-upload v-model:file-list="spuImageList" :action="setting.fileUpload" list-type="picture-card"
          :on-preview="handlePictureCardPreview" :before-upload="handlBeforUpload" :on-remove="handleRemove"
      ...
      <el-form-item label="SPU销售属性">
        <el-select v-model="saleAttrAndValueName"
          :placeholder="unusedSaleAttr.length ? `还剩${unusedSaleAttr.length}个未选择` : `销售属性已全选`">
          <el-option v-for="item in unusedSaleAttr" :key="item.id" :label="item.name" :value="`${item.id}:${item.name}`"
            :disabled="unusedSaleAttr.length ? false : true">
          </el-option>
        </el-select>
        <el-button @click="addSale" :disabled="saleAttrAndValueName ? false : true" type="primary" icon="Plus"
          style="margin:10px">添加销售属性</el-button>
        <el-table border :data="SaleAttrList">
          <el-table-column type="index" label="序号" width="80" header-align="center" />
          <el-table-column label="属性名" width="120" prop="saleAttrName" header-align="center" />
          <el-table-column label="属性值" header-align="center">
            <template #="{ row, $index }">
              <el-tag style="margin: 0 5px;" v-for="(item, index) in row.spuSaleAttrValueList" :key="index" closable
                @close="row.spuSaleAttrValueList.splice(index, 1)">
                {{ item.saleAttrValueName }}
              </el-tag>
              <el-input v-model="row.saleAttrValue" @blur="toLook(row)" v-if="row.flag" placeholder="请输入属性值" size="small"
                style="width: 100px;" />
              <el-button v-else type="success" size="small" icon="Plus" @click="addSaleValue(row)"></el-button>
            </template>
          </el-table-column>
      ...
      <el-form-item>
        <el-button :disabled="SaleAttrList.length > 0 ? false : true" type="primary" @click="saveAttr">确定</el-button>
