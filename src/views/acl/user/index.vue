<template>
  <div>
    <SelectKey label="用户名" placeholder="请输入搜索姓名关键字" v-model:modelValue="keyword" @handler-select="searchKeyword"></SelectKey>
    <el-card style="margin: 10px 0;">
      <!-- 操作按钮 -->
      <el-button type="primary" @click="addUser">添加</el-button>
      <el-button :disabled="selectRoleUserList.length? false : true" @click="openMessageBox" type="danger">批量删除</el-button>
      <!-- 用户列表 -->
      <el-table @selection-change="handleSelectionChange" :data="userList" border style=" margin: 10px auto; width: 100%"
        height="310">
        <el-table-column type="selection" align="center"></el-table-column>
        <el-table-column type="index" label="序号" width="60" align="center"></el-table-column>
        <el-table-column prop="id" label="ID" width="60" align="center"></el-table-column>
        <el-table-column prop="username" width="140" label="用户名字" align="center" show-overflow-tooltip></el-table-column>
        <el-table-column prop="name" width="140" label="用户昵称" align="center" show-overflow-tooltip></el-table-column>
        <el-table-column prop="roleName" width="100" label="用户角色" align="center" show-overflow-tooltip></el-table-column>
        <el-table-column prop="createTime" width="166" label="创建时间" align="center"
          show-overflow-tooltip></el-table-column>
        <el-table-column prop="updateTime" width="166" label="更新时间" align="center"
          show-overflow-tooltip></el-table-column>
        <el-table-column label="操作" align="center" fixed="right" width="270">
          <template #="{ row, $index }">
            <el-button type="primary" size="small" icon="Place" @click="setRole(row)">分配角色</el-button>
            <el-button type="info" size="small" icon="Edit" @click="editUser(row)">编辑</el-button>
            <el-popconfirm width="250" :title="`确定要删除${row.username}吗？`" @confirm="removeRole(row)">
              <template #reference>
      ...
      <!-- 分页器 -->
      <el-pagination v-model:current-page="pageData.page" v-model:page-size="pageData.limit" :page-sizes="[5, 7, 15]"
        background="background" layout="prev, pager, next, jumper, ->, sizes, total" :total="pageData.total"
        @current-change="getUserList" @size-change="handlerLimit" />
        ...
        <template #default>
          <el-form :rules="userRules" ref="userFormRef" :model="userAccount">
            <el-form-item label="用户姓名" prop="username">
              <el-input v-model.trim.lazy="userAccount.username" placeholder="请填写用户姓名" clearable></el-input>
            </el-form-item>
            <el-form-item label="用户昵称" prop="name">
              <el-input v-model.trim.lazy="userAccount.name" placeholder="请填写用户昵称" clearable></el-input>
            </el-form-item>
            <el-form-item label="用户密码" prop="password" v-if="!userAccount.id">
              <el-input v-model.trim.lazy="userAccount.password" placeholder="请填写用户密码" clearable show-password></el-input>
            </el-form-item>
          </el-form>
        </template>
        <template #footer>
          <div style="flex: auto">
            ...
            <el-button type="primary" @click="saveUserAccount(userFormRef)">确定</el-button>
          </div>
        </template>
      </el-drawer>
        ...
        <template #default>
          <el-form :model="userAccount">
            ...
            <el-form-item label="职位列表">
              <el-checkbox v-model="roleCheckAll" :indeterminate="isIndeterminate" @change="handleCheckAllChange">
                全选
              </el-checkbox>
              <el-checkbox-group v-model="nowRoleList" @change="handleCheckedCitiesChange">
                <el-checkbox v-for="(role, index) in roleList" :key="index" :label="role">
                  {{ role.roleName }}
                </el-checkbox>
              </el-checkbox-group>
            </el-form-item>
...
import { reqAllUser, reqAddOrUpdateUser, reqAllRole, reqSetUserRole, reqRemoveSelectUser, reqRemoveUser } from '@/api/acl/user/index'
import type { UserListResponseData, UserData, UserRoleResponseData, allRole, nowRole, SetRoleData } from '@/api/acl/user/type'
...
let userRules = reactive<FormRules>({
  ...
  password: [
    ...
    {
      pattern: /^(?![\d]+$)(?![a-zA-Z]+$)(?![^\da-zA-Z]+$)([^\u4e00-\u9fa5\s]){6,10}$/,
      message: '6-10位英文字母、数字或者符号（除空格），且字母、数字和标点符号至少包含两种',
      trigger: 'blur'
    }
  ]
})
...
const getUserList = async (pager = 1) => {
  ...
  // keyword.value = word
  // 发请求
  let res: UserListResponseData = await reqAllUser(pageData.page, pageData.limit, keyword.value)
  if (res.code == 200) {
    userList.value = res.data.records.map((item: UserData) => {
      // 遍历对象的每一个键值对
      let transformedItem = Object.entries(item).reduce((acc, [key, value]) => {
        // 判断值是否为空
        let transformedValue = value !== '' && value !== null ? value : '暂未分配'
        // 将键值对添加到累积的结果中
...
const saveUserAccount = async (formEl: FormInstance | undefined) => {
  ...
  if (res.code == 200) {
    ElMessage.success(userAccount.id ? `${userAccount.username},修改成功` : `${userAccount.username},创建成功`)
  } else if (res.data == '系统数据，不能操作') {
    ElMessage.error('你的权限不够,不能修改!')
  } else {
    ElMessage.error(userAccount.id ? `${userAccount.username},修改失败` : `${userAccount.username},创建失败`)
  }
  drawer.value = false
...
const openMessageBox = async () => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      let idList: number[] = selectRoleUserList.value.map(item => (item.id as number))
